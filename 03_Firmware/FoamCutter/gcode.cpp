...
void gc_init()
{
  ...
  gc.tool_state = 5;                          // for M5 = Tool off
  gc.tool_pwr   = settings.tool_pwr;          // stored tool power
...
/// 8c1
void gc_set_current_position(int32_t x, int32_t y, int32_t u, int32_t z)
{
  ...
  gc.position[U_AXIS] = z/settings.steps_per_mm[U_AXIS];
  
 // gc.position[U_AXIS] = u/to_degrees(settings.steps_per_mm[U_AXIS]); /// steps_per_degrees
}
...
/// 8c0
  ...
  while(next_statement(&letter, &value, line, &char_counter)) {
    ...
    switch(letter) {
      ...
      case 'M':
        ...
        // Set 'M' commands use for hotwire control
        switch(int_value) {
          ...
          case 3:          gc.tool_state = 3; break;
          case 4:          gc.tool_state = 4; break;
          case 5:          gc.tool_state = 5; break;    
          default: FAIL(STATUS_UNSUPPORTED_STATEMENT);
        }
        break;
    }
  ...
  while(next_statement(&letter, &value, line, &char_counter)) {
    switch(letter) {
      ...
      case 'R':
        ...
        break;
      case 'S':         // S value only for hotwire uses
        if (value < 0) {
  ...
  // [M3,M4,M5]: Perform tool operations.
  if ( bit_istrue(modal_group_words,bit(MODAL_GROUP_7))) {
    // Axis are not moved, so tools need to be operated manually
    protocol_buffer_synchronize();
    // [M3,M4,M5]: Update tool state
    tool_run(gc.tool_state, gc.tool_pwr);
  }
  ...
  switch (non_modal_action) {
    ...
    case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1:
      ...
      if (axis_words) {
        ...
        mc_line(target[X_AXIS], target[Y_AXIS], target[U_AXIS], target[Z_AXIS], settings.default_seek_rate, false, C_LINE, gc.tool_state, gc.tool_pwr);
      }
      ...
      ///8c1  : line
      mc_line(coord_data[X_AXIS], coord_data[Y_AXIS], coord_data[U_AXIS], coord_data[Z_AXIS], settings.default_seek_rate, false, C_LINE, gc.tool_state, gc.tool_pwr);
...
/// 8c1 :line
          mc_line(target[X_AXIS], target[Y_AXIS], target[U_AXIS], target[Z_AXIS],
                (gc.inverse_feed_rate_mode) ? inverse_feed_rate : gc.feed_rate, gc.inverse_feed_rate_mode, C_LINE, gc.tool_state, gc.tool_pwr);
        }
        break;
      ...
      center of arc when h_x2_div_d is positive ->  x <----- | -----> x <- center of arc when h_x2_div_d is negative
          ...
          mc_arc(gc.position, target, offset, gc.plane_axis_0, gc.plane_axis_1, gc.plane_axis_2,
            ...
            r, isclockwise, gc.tool_state, gc.tool_pwr);
        }
        break;
    }
